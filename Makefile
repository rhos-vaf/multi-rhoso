...
verify-nncp: ## Verify NNCP configuration and IP addresses
	...
	@oc get nncp
	@echo ""
	@echo "IP Addresses on CRC Node (enp6s0.20 - InternalAPI):"
	@oc -n default debug node/crc -- chroot /host ip addr show enp6s0.20 2>/dev/null | grep "inet " || echo "Failed to get IPs"
